...
HTTPS_GIT := https://github.com/andromedaprotocol/wasmd.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=wasm \
		  ...
		  -X github.com/andromedaprotocol/wasmd/app.Bech32Prefix=wasm \
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/andromedaprotocol/wasmd
